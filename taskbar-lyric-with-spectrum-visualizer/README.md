...
- 📊 **实时频谱显示** — 订阅系统音频，在浮窗底部绘制混合样式（波形线 + 柱状）频谱，配色、透明度、填充与背景光晕跟随「频谱可视化」插件（主插件采样其渲染画面与图层样式自动同步），渲染 30 FPS
...
- 浮窗频谱为固定混合样式（波形线 + 柱状叠加），渲染 30 FPS；配色、透明度、填充率与背景光晕由主插件周期性采样官方频谱画面与图层样式自动跟随：
  - 颜色：采样「频谱可视化」实际渲染的频谱 canvas 还原三档渐变
  - 透明度/填充：读取官方图层 `--echo-spectrum-opacity` / `--echo-spectrum-fill` 样式变量
  - 背景光晕：读取官方图层 `data-backdrop` 开关，开启时按官方算法绘制整画布渐变光晕 + 暗色底部渐变
  - 更新延迟约 2–3.5s（主入口每 1.5s 采样写入，浮窗每 2s 轮询读取）；未播放/暂停时保留上次配色
...
| 频谱绘制 | Canvas 2D，DPR 自适应，混合样式（波形线 + 柱状），30 FPS；配色/透明度/填充/背景光晕采样官方插件画面与图层样式自动跟随 |
