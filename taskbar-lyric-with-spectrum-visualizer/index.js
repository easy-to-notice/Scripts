...
const broadcastSettings = () => {
  ...
  try {
    ...
    console.warn("[taskbar-lyric-spectrum] 同步设置失败", error);
  }
};
    ...
    if (payload.type === "command") {
      if (payload.command === "dislikeFm" && ctx.player?.dislikePersonalFm) {
        ctx.player.dislikePersonalFm().catch((e) => {
          console.warn("[taskbar-lyric-spectrum] dislikeFm via channel failed:", e);
        });
      }
      return;
    }
...
const createPreviewComponent = (ctx) =>
  ctx.vue.defineComponent({
    name: "TaskbarLyricSpectrumPreview",
    props: {
...
const createSettingsComponent = (ctx) =>
  ctx.vue.defineComponent({
    name: "TaskbarLyricSpectrumSettings",
    setup() {
      ...
      const saveDraft = async () => {
        ...
        try {
          ...
          message.value = "设置已保存";
          ctx.toast.success("任务栏歌词+频谱设置已保存");
        } catch (error) {
      ...
      return () =>
        h("div", { class: "tb-lyric-settings" }, [
          ...
          renderSection("窗口", "窗口自动贴靠任务栏。拖动百分比偏移手动调整位置（自动适配屏幕分辨率）", [
            ...
            renderSwitchRow("showLockButton", "显示解锁按钮", "关闭后悬停控件栏不显示解锁/锁定按钮"),
            renderSwitchRow("manualAdjust", "手动调整", "勾选后可通过下方偏移滑块微调浮窗位置"),
            renderSwitchRow("clickToShowMain", "点击空白处唤出主窗口", "点击歌词浮窗空白处将 EchoMusic 主窗口显示到前台"),
...
const registerSettings = (ctx) => {
  ...
  settingsDispose = ctx.ui.settings.define({
    title: "任务栏歌词+频谱",
    description: "在任务栏上方显示歌词与实时频谱的透明浮窗",
